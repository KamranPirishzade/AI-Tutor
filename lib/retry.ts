 ...
 * (not a guessed exponential backoff), on a transient "model overloaded"
 * 503 a couple of times, and once on a plain network blip. Never retries a
 * daily-quota error or a rate limit whose suggested wait is too long to
 * fit — those are surfaced immediately instead. */
